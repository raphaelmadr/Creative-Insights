...
* **Prevenção de Ciclos de Upload Infinitos (Vercel Blob):** Corrigido um bug lógico onde imagens já consolidadas na Vercel (onde `imageUrl === thumbnailUrl`) eram erroneamente marcadas como "borradas" e sofriam re-upload a cada sincronização. Imagens que já apontam para `public.blob.vercel-storage.com` agora são protegidas e ignoradas pelo validador.
