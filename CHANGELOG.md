...
* **Autenticação compatível com disparadores de hospedagem:** O endpoint aceita `Authorization: Bearer <CRON_SECRET>` ou, para painéis que só permitem colar uma URL, `?secret=<CRON_SECRET>`. Aceita GET e POST. Com `?force=1` o portão de intervalo é ignorado, para testar o disparo na hora.
