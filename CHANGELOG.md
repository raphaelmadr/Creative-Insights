...
### 🎯 20. Filtro de Coorte Restrito a Lançamentos (Criativos & Equipe)
As regras de negócio visuais foram ajustadas para garantir que as visualizações respeitem estritamente as novidades lançadas no período.
* **Filtro Duro no Banco de Dados (`/api/db-ads`):** Adicionada trava baseada na coluna `createdTime` diretamente na Query. Ao selecionar um mês no Dashboard Inicial, as colunas de "Winners" e "Super Winners" ocultam imediatamente peças que foram herdadas de meses anteriores (mesmo que tenham consumido verba no mês atual). O foco da tela inicial torna-se 100% analítico sobre a "safra de criações" daquele período específico.
* **Fim da Tolerância de 5 Dias (Relatórios de Equipe):** A tela de relatórios da Equipe (`/api/reports/creators`) possuía uma tolerância silenciosa que englobava o faturamento de criativos lançados nos 5 últimos dias do mês anterior. Essa janela foi removida por completo. O cálculo de faturamento da equipe agora é restrito, isolando cirurgicamente peças criadas do primeiro (01 à meia-noite UTC) ao último segundo do mês selecionado.
